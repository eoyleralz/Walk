...
(function($) {
  ...
  luminateExtend.api.bind = function(selector) {
	if (console) { console.log('binding form'); }
    selector = selector || 'form.luminateApi';
    if (console) { console.log($(selector)); }
  ...
  luminateExtend.api.getAuth = function(options) {
    ...
    if (console) { console.log('GetAuth Start'); }
    ...
    if(luminateExtend.api.getAuthLoad) {
      ...
      if(settings.useCache && 
        ...
        var getAuthCallback = function(globalData) {
		  if (console) { console.log('getAuthCallback1'); }
        ...
        if(luminateExtend.global.supportsCORS) {
		  if (console) { console.log('supportsCORS'); }
          $.ajax({
            ...
            success: function(data) {
			  if (console) { console.log('supportsCORS AJAX Success'); }
		  ...
		  if (console) { console.log('not supportsCORS'); }
          $.ajax({
    ...
    if(luminateExtend.global.path.nonsecure && 
      ...
      if(useAjax) {
        doRequest = function() {
          ...
          if(settings.requiresAuth && settings.data.toString().indexOf('&' + luminateExtend.global.auth.type + '=') === -1) {
  ...
  luminateExtend.utils = {
	...
	window.getUser = function() {
	  var getUserCallback = function(data) {
		if (console) { console.log('getUserCallback'); console.log(data); }
		if(data.getConsResponse && data.getConsResponse.name) {
		  $('#login-form').hide();
		  $('#login-form').after('<p class="navbar-text pull-right" id="welcome-back">' + 
										 'Welcome back' + ((data.getConsResponse.name.first) ? (', ' + data.getConsResponse.name.first) : '') + '! ' + 
										 '<a class="logout" href="' + luminateExtend.global.path.nonsecure + 'UserLogin?logout=&NEXTURL=' + encodeURIComponent(window.location.href) + '">Logout</a>' + 
									   '</p>');
	  ...
	  if (console) { console.log('getUser'); }
	  luminateExtend.api({
		api: 'cons', 
		data: 'method=getUser', 
		requiresAuth: true,
...
var app = {
    ...
    receivedEvent: function(id) {
		...
		/* configuration of API helper library */
		luminateExtend({
			apiKey: 'ALZADARDA', 
			path: {
		...
		if (luminateExtend.global.auth.token == null) {
			if (console) { console.log('getAuth'); console.log(luminateExtend.global.auth.token); }
			luminateExtend.api.getAuth({callback: $.noop, useCache: true, useHTTPS: true});
		}
		...
		/* handle the login form in the site's header */
		...
		window.loginCallback = {
		  error: function(data) {
			if (console) { console.log('loginCallback Error'); console.log(data); }
			if($('#login-error').length > 0) {
			  $('#login-error').html(data.errorResponse.message);
			}
		  }, 
		  success: function(data) {
			if (console) { console.log('loginCallback Success'); console.log(data); }
			getUser();
		  }
		};
    }
